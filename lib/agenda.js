...
Agenda.prototype.saveJob = function(job, cb) {
  if (!job.attrs._id) {
    this.log('saving new job');
  } else {
    this.log('saving job %s', job.attrs._id);
  }
...
Agenda.prototype.start = function() {
  this.log('start processing jobs');
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  this.log('looking for job to lock');
  ...
  if ( this._mdb.s.topology.connections().length === 0 ) {
    ...
    this._collection.findAndModify(
      ...
      function( error, result ){
          ...
          if (jobs) {
            self.log('locked job %s at %s', jobs.attrs._id, jobs.attrs.lockedAt.toISOString())
          }
          cb( error, jobs );
      }
    );
...
function processJobs(extraJob) {
  this.log('processing jobs in queue');

  if (!this._processInterval) {
    this.log('_processInterval not set');
    return;
  }
  ...
  if (!extraJob) {
    ...
    this.log('locking job %s on the fly at %s', extraJob.attrs._id, now.toISOString());
  ...
  function enqueueJobs(jobs) {
    ...
    for (job in jobs) {
      ...
      self.log('enqueued job %s at index %d', job.attrs._id, job_index);
    }
  }

  function jobQueueFilling(name) {
    self.log('filling job queue');
  ...
  function jobProcessing() {
    self.log('processing jobs in queue');
    ...
    if (job.attrs.nextRunAt < now) {
      ...
      var timeout = job.attrs.nextRunAt - now;
      self.log('scheduling to run job %s in %dms', job.attrs._id, timeout);
      setTimeout(runOrRetry, timeout);
    }

    function runOrRetry() {
      self.log('running job %s', job.attrs._id);
      ...
      if (self._processInterval) {
        if (jobDefinition.concurrency > jobDefinition.running &&
          ...
          if (job.attrs.lockedAt < lockDeadline) {
            self.log('lock expired for job %s', job.attrs._id);
            jobProcessing();
            return;
          }
          ...
          self.log('putting job %s back in queue', job.attrs._id);
          jobQueue.push(job);
        }
      }
    }
  ...
  function processJobResult(err, job) {
    self.log('done running job %s', job.attrs._id);
...
Agenda.prototype.log = function(msg) {
  if (this._name) {
    msg = '[' + this._name + '] ' + msg;
  }

  debug.apply(null, [ msg ].concat(Array.prototype.slice.call(arguments, 1)));
};
